...
bool control_utils_pid_run(float error, control_utils_pid_data_t *const pid_data) {
    ...
    const bool pid_config_initialized = (pid_data_initialized) && (pid_data->config != NULL) && (pid_data->config->dt > 0.0);
    ...
    if (pid_data_initialized && pid_config_initialized) {
        ...
        const float derivative = (error - pid_data->last_error) / pid_data->config->dt;
