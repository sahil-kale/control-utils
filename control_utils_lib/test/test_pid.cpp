...
TEST(pid_test, check_init) {
    ...
    CHECK_EQUAL(true, control_utils_pid_init(&pid_data, &pid_config, NULL));
...
TEST(pid_test, check_null) { CHECK_EQUAL(false, control_utils_pid_init(NULL, NULL, NULL)); }
...
TEST(pid_test, check_run_ki_max_windup) {
    ...
    control_utils_pid_data_t pid_data;
    bool ret = control_utils_pid_init(&pid_data, &pid_config, &pid_limits);
    CHECK_EQUAL(true, ret);

    float integral = 0.0;
    for (uint8_t i = 0; i < 10; i++) {
        const float err = 10.0;
        ret = control_utils_pid_run(err, &pid_data);
...
TEST(pid_test, check_run_kd) {
    ...
    ret = control_utils_pid_run(20.0, &pid_data);
    CHECK_EQUAL(true, ret);
    DOUBLES_EQUAL((10 / 0.1 * 20), pid_data.output, 0.01);
}
...
TEST(pid_test, invalid_params) {
    CHECK_EQUAL(false, control_utils_pid_run(10.0, NULL));

    // Test with uninitialized pid_data
    control_utils_pid_data_t pid_data;
    pid_data.initialized = false;
    CHECK_EQUAL(false, control_utils_pid_run(10.0, &pid_data));
}